...
function CollectionPage() {
  ...
  const fetchRecords = async () => {
    ...
    try {
      const res = await fetch(`https://document-verify-backend.onrender.com/api/get_all_document?employee_id=${employee_id}`,{
        method: 'GET'
      });
      if (!res.ok) {
      ...
      if (data?.ok && Array.isArray(data.documents)) {
        const formattedFiles = data.documents.map(record => ({
          ...
          imageUrl: `https://document-verify-backend.onrender.com/thumb_text?text=${encodeURIComponent(record.result_json?.title || 'Doc')}`,
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (file.original_name) {
                            window.location.href = `https://document-verify-backend.onrender.com/download/${encodeURIComponent(file.original_name)}`;
                          } else {
