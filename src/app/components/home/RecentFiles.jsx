...
export default function RecentFiles({
    ...
    const API_URL = process.env.PY_BACKEND_URL || 'https://document-verify-backend.onrender.com';
            ...
            saved.length === 0 ? (
                ...
                <div className = "grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6" > {
                    saved.map((rec) => (
                        ...
                        <Image src = {
                            `https://document-verify-backend.onrender.com/thumb_text?text=${encodeURIComponent(rec.result_json?.title || 'Doc')}`
                        }
                        alt = {
                                ...
                                onClick = {
                                    () => {
                                        if (rec.original_name) {
                                            window.location.href = `https://document-verify-backend.onrender.com/download/${encodeURIComponent(rec.original_name)}`;
                                        } else {
